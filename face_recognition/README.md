...
## Introduction
For the face recognition we use arcface_torch from InsightFace: https://github.com/deepinsight/insightface/tree/master/recognition/arcface_torch

Please check ``inference.py`` for running arcface_torch on the ``personen`` folder. This results in ``features.pkl`` containing a pandas DataFrame with all the feature vectors, folder names and file names: 'feature', 'folder', 'filename'.


Below is more information from the original repository:


## ArcFace
The "arcface_torch" (https://github.com/deepinsight/insightface/) repository is the official implementation of the ArcFace algorithm. It supports distributed and sparse training with multiple distributed training examples, including several memory-saving techniques such as mixed precision training and gradient checkpointing. It also supports training for ViT models and datasets including WebFace42M and Glint360K, two of the largest open-source datasets. Additionally, the repository comes with a built-in tool for converting to ONNX format, making it easy to submit to MFR evaluation systems.
