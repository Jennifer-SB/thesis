...
class RKDDataset:
        ...
        with open(self.xml_file, "r", encoding="utf-8") as f:
            with tqdm(total=file_size, unit="B", unit_scale=True, desc="Parsing XML") as pbar:
                for line in f:
                    ...
                    # There could be multiple people identified in a painting, so a dict is made
                    ...
                    # again only one variable telkens?
                    ...
                    # === GENRE === (? also only one variable telkens?)
                    elif '<genre tag=' in s:
                        inside_genre = True
                    ...
                    # === DRAGER === (? telkens maar 1 variabele?)
                    ...
                    # === RKD_ALGEMENE_TREFWOORDEN (multi-value) ===
    ...
    def sitter_portrait_counts(self) -> Counter:
        ...
        self._check_parsed()
        counts = Counter()
        # records = dictionary with key = priref, value = dict with all the info.
        # so rec is one artwork(info) at a time (103.637 artworks)
        for rec in self.records.values():
            # loops through all the persons in the painting (identified)
            # ! HERE !
